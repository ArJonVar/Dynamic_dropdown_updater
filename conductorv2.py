...
class ConductorV2:
    ...
    def gather_column_ids(self):
        ...
        try:
            ...
            print("failed to find column_ids, check that the column names have not changed and try again")
    ...
    def generate_conductor_dict(self):
        ...
        print(f'{self.timestamp()} gathering data...')
        row_objects = []
    ...
    def ssdata_audit(self, row_list, location_str):
        ...
        print(f'{self.timestamp()} auditing {location_str.lower()} data...')
    ...
    def extract_name_n_email_list(self):
        try:
            ...
            if self.log and self.error_message == False:
                self.log.log(self.row_id, "EXTRACTING EMAILS ERROR: Check that your source is a CONTACT column, and that each value is a CONTACT, then check that the desired column in your destination is CONTACT column")
    ...
    def run_dynamic_dropdowns(self, row_list):
        ...
        print(f'{self.timestamp()} initiating column updates...')
        for count, row_data in enumerate(row_list):
            try:
                print(f'{self.timestamp()} updating row {row_data.get("ROW_ID")} ({count+1} of {str(len(row_list))})')
